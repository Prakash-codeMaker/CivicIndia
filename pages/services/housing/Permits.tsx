...
const PermitsPage: React.FC = () => {
  ...
  const meta = (servicesMetadata as any).services?.housing?.sections?.permits;
  ...
  return (
    <div className="py-16">
      <div className="container mx-auto px-4">
        ...
        <h1 className="text-2xl font-bold text-white mb-4">{meta?.title ?? 'Building Permits'}</h1>
        ...
        <div className="mt-6">
          <button onClick={() => navigate('/services/housing/Apply')} className="px-4 py-2 bg-glow-blue rounded text-white">Start Permit Application</button>
        </div>
      </div>
    </div>
  );
